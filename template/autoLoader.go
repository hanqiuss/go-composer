...
func Generated(l map[string]*repositories.JsonPackage) {
	ps := make([]parse.Dir, 0)
	...
	for name, pkg := range l {
		...
		for k, v := range pkg.AutoLoad {
			if k == "exclude-from-classmap" {
				exc[name] = make([]string, 0)
				for _, vv := range v.([]interface{}) {
					exc[name] = append(exc[name], vv.(string))
