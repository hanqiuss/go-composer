...
return array({{range $k,$v := .RootPsr}}
    '{{$k}}' => array({{range $_,$s := $v}} $baseDir . '/{{$s}}',{{end}}),{{end}}
);`
...
func parsePsr(d interface{}) map[string][]string {
	var psr = make(map[string][]string)
	for name, paths := range d.(map[string]interface{}) {
		switch paths.(type) {
		case []interface{}:
			for _, path2 := range paths.([]interface{}) {
				switch path2.(type) {
				case string:
					path5 := path2.(string)
					if path5 != "" {
						psr[name] = append(psr[name], path5)
					}
				}
			}
		case string:
			path3 := paths.(string)
			if path3 != "" {
				psr[name] = append(psr[name], path3)
			}
		}
	}
	return psr
